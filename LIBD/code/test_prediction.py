...
def report_prop_method_LIBD (folder, tissueID, name, dataSection2, traindata, Val_loader, coloruse, outname = ""):
    ...
    with torch.no_grad():
        for i, img in enumerate(Val_loader):
            ...
            if (logitsvalue[class_num-2] == 1):
                ...
                #print(logitsvalue_max)
                
                prb = np.diff(logitsvalue_max)
                #print(prb)
                # prbfull = np.insert(-prb[0], 0, 1 -logitsvalue[0,0], axis=0)
                prbfull = -prb.copy()
                #print(prbfull,prbfull.max())
                #print(np.where(prbfull == prbfull.max()))
                coords_predict[i] = np.where(prbfull == prbfull.max())[0].max() + 1
                #print(payer_prob[i,2:].shape,prbfull.shape)
                payer_prob[i,2:] = prbfull
                
                #print("logitvalue",logitsvalue,logitsvalue.max())
                #print("logitvalue_min",logitsvalue_min)
                #print("logitvalue_max",logitsvalue_max)
                #print("prbfull",prbfull,prbfull.max())
                #print(np.where(prbfull == prbfull.max())[0].max() + 1)
    ...
    print(payer_prob[:,0].shape)
    print(payer_prob[:,1].shape)
...
def main():
	...
	dataSection1 = sc.read(r"C:\Users\Agrya\Documents\code\data\data_151673.h5ad")
	dataSection2 = sc.read(r"C:\Users\Agrya\Documents\code\data\data_151676.h5ad")
	dataSection3 = sc.read(r"C:\Users\Agrya\Documents\code\data\data_151507.h5ad")
	...
	# test on tissue ID: 
	testdata = dataSection2
	tissueID = 151676
	traindata = dataSection1
	#beta = 0.005
	#nrep=2
	coloruse = None
	...
	#
	report_prop_method_LIBD(folder = r"C:\Users\Agrya\Documents\code\output\LIBD\Prediction", tissueID = tissueID,
	...
	# Top-1 Accuracy and Top-2 Accuracy
	path = r"C:\Users\Agrya\Documents\code\output\LIBD\Prediction151676\data_gen_layer_0.005_n2_probmat.csv"
	accuracy_summary = overall_acc_summary(path)



	# test on tissue ID: 151507
	testdata = dataSection3
	tissueID = 151507
	traindata = dataSection1
	#beta = 0.005
	#nrep=2
	...
	# Top-1 Accuracy and Top-2 Accuracy
	path = r"C:\Users\Agrya\Documents\code\output\LIBD\Prediction151507\data_gen_layer_0.005_n2_probmat.csv"
	accuracy_summary = overall_acc_summary(path)
